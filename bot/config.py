...
try:
    ...
    DEV = 1287276743
    OWNER = config("OWNER")
    ffmpegcode = ["-preset faster -c:v libx265 -s 720x480 -x265-params 'bframes=8:psy-rd=1:ref=3:aq-mode=3:aq-strength=0.8:deblock=1,1' -metadata 'title=Encoded By AnimeColony Telegram Channel' -metadata:s:v title="@animecolony" -metadata:s:a title="@animecolony" -metadata:s:s title="@animecolony" -pix_fmt yuv420p -crf 31 -c:a libopus -b:a 32k -c:s copy -map 0 -ac 2 -ab 32k -vbr 2 -level 3.1 -threads 1 -vf "[in] drawtext=fontfile=arial.ttf: fontsize=24: fontcolor=white: x=(w-text_w)/23: y=h/28: text='@animecolony': enable='between (t,500,600)' ,
